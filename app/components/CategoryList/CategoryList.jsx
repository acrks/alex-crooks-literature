...
import { getCatThemeColor } from '@/app/api/functions'
  ...
  return (
    <div className='mt-[30px] flex-row justify-start text-left'>
        ...
        <div className='flex justify-evenly mt-[50px] gap-x-1'>
            {categories?.map((item) => (
                <Link className={`${getCatThemeColor('bg', item.slug)} rounded-lg drop-shadow-[0px_0px_5px_rgba(253,224,71,0.25)] hover:scale-[1.025] hover:drop-shadow-[0_0_7px_rgba(253,224,71,0.5)] transition-all duration-200`} href={`/writings/${item.slug}`} key={item.id}>
